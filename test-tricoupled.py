...
from torchvision.utils import save_image
from models.models import PatchSampleF, BoundaryGenerator,BoundaryGenerator2,BoundaryGenerator3
...
def test(attr,
    ...
    G2 = BoundaryGenerator3(fix_len=7).cuda()
    G2.load_state_dict(torch.load('checkpoints/ffhq/celeba-test/%s/030000.pt' % attr)["G"])
    ...
    attr1,attr2,attr3=attr.split('-')
    save_dir = 'test-results/%s/%s' % (attr,attr1)
    os.makedirs(save_dir, exist_ok=True)
    imgs = []
    bs = 8
    cnt = 0
    ...
    save_dir = 'test-results/%s/%s' % (attr, attr2)
    os.makedirs(save_dir, exist_ok=True)
    imgs = []
    bs = 8
    cnt = 0
    ...
    save_dir = 'test-results/%s/%s' % (attr, attr3)
    os.makedirs(save_dir, exist_ok=True)
    imgs = []
    bs = 8
    cnt = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--attr', type=str, default='Smiling-Old-Bald')
